...
class BaseLLM:
    ...
    def batched_generate(
        ...
        # use left padding so that newly generated tokens apear after the prompt

        self.tokenizer.padding_side = "left"
        self.tokenizer.pad_token = self.tokenizer.eos_token

        # tokenize prompts and move tensors to the model device
        ...
        # enable samplings for RFT when temprature > 0
        if temperature > 0:
          gen_kwargs.update({
            "do_sample": True,
        ...
        #allow multiple generation per promt
        if num_return_sequences is not None:
            gen_kwargs["num_return_sequences"] = num_return_sequences

        # geneate sequences from the model
        ...
        #regroup outputs if multiple sequences per prompt
        if variants > 1:
            return [
