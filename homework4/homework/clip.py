...
class CLIP(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Step 1: Get image features from vision encoder
        ...
        # Step 2: Get text features from text encoder
...
def compute_clip_loss(
    ...
    # Step 1: Unpack model outputs
    # img_emb → image embeddings
    # txt_emb → text embeddings
    # logits  → similarity matrix (image vs text)
    img_emb, txt_emb, logits = outputs

    # Step 2: Get batch size (number of samples)
    n = logits.size(0)

    # Step 3: Create target labels
    # For CLIP, correct pairs are on the diagonal:
    # image[i] matches text[i]
    target = torch.arange(n, device=logits.device)

    # Step 4: Compute image-to-text loss
    # Each image should match the correct text (same index)
    ...
    # Step 5: Compute text-to-image loss
    # Each text should match the correct image
    ...
    # Step 6: Final loss = average of both directions
    # This makes training symmetric and more stable
