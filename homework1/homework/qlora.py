...
class QLoRALinear(Linear4Bit):
    def __init__(
        ...
        #LoRA
        self.ql_a = torch.nn.Linear(in_features, lora_dim, bias=False, dtype=torch.float32)
        self.ql_b = torch.nn.Linear(lora_dim,  out_features, bias=False, dtype=torch.float32)

        # initialize layers
        torch.nn.init.kaiming_uniform_(self.ql_a.weight, a=5**0.5)
        torch.nn.init.zeros_(self.ql_b.weight)
        self.linear_dtype = torch.float32

        self.scaling = 1 / lora_dim
        #raise NotImplementedError()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # TODO: Forward. Make sure to cast inputs to self.linear_dtype and the output back to x.dtype
        orig_output = super().forward(x)
        ql_input_32 = x.to(self.linear_dtype)
        ql_update = self.ql_b(self.ql_a(ql_input_32))
        #update32 = ql_update.to(x.dtype)
        update32 = (ql_update * self.scaling).to(x.dtype)
        return orig_output + update32
        #raise NotImplementedError()
