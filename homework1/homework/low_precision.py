...
class Linear4Bit(torch.nn.Module):
    ...
    def _load_state_dict_pre_hook(
        ...
        if f"{prefix}weight" in state_dict:
            ...
            quantized, scale = block_quantize_4bit(weight.view(-1), self._group_size)
            self.weight_q4  .copy_(quantized)
            self.weight_norm.copy_(scale)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            full_weight = block_dequantize_4bit(self.weight_q4, self.weight_norm)
            full_weight = full_weight.view(self._shape).to(x.dtype)
            return torch.nn.functional.linear(x, full_weight, self.bias)  
            #raise NotImplementedError()
